  ...
  useEffect(() => {
    (async () => {
      const response = await http.get<BrandInfo>(
        `http://192.168.100.159:8081/api/product/brands/${param.brandName}`
      );
    ...
    (async () => {
      if (!isLast) {
        const response = await http.get<PaginationResponse<ProductItem>>(
          `http://192.168.100.159:8081/api/product/brands/items/${param.brandName}?page=${page}&size=${PAGE_SIZE}`
        );
        if (response !== undefined) {
          if (response.status === 200) {
    ...
    <>
      <Category>
        <section>
          ...
          {brand && (
            <img
              src={`http://192.168.100.159:8081/api/product/files/${brand.imageUuidName}`}
            />
          )}
